...
export default function AdminCategoryModal({ 
  ...
  async function swapCategories(categoryId1: string, categoryId2: string) {
    ...
    try {
      ...
      if (!cat1 || !cat2) {
        throw new Error('Kategori tidak dijumpai');
      }

      // Swap display_order values
      const tempOrder = cat1.display_order;
      cat1.display_order = cat2.display_order;
      cat2.display_order = tempOrder;

      // Update both categories in Supabase
      const supabase = createClient();
      
      // Update first category
      const { error: error1 } = await supabase
        .from('categories')
        .update({ display_order: cat1.display_order })
        .eq('id', cat1.id);

      // Update second category
      const { error: error2 } = await supabase
        .from('categories')
        .update({ display_order: cat2.display_order })
        .eq('id', cat2.id);

      if (error1 || error2) {
        throw new Error(error1?.message || error2?.message || 'Gagal mengemas kini susunan kategori');
      }

      // Refresh the categories list
      await fetchCategories();
      setSuccessMessage('Susunan kategori berjaya dikemas kini');
    } catch (error) {
      console.error('Error swapping categories:', error);
      setError(error instanceof Error ? error.message : 'Gagal menukar susunan kategori');
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleMoveUp = (index: number) => {
    ...
    swapCategories(categoryId, prevCategoryId);
  };
